...
def _default_theme_dir() -> Path:
    return Path(os.environ.get("XDG_CONFIG_HOME", Path.home() / ".config")) / "theme-engine" / "themes"
...
class ThemeEditor:
    ...
    def __post_init__(self) -> None:
        ...
        loaded["name"] = self.theme_name  # heal old files whose inner name drifted
    ...
    def from_data(cls, name: str, data: dict[str, Any], theme_dir: Path | None = None,
        ...
        obj.max_history = 100
        data = deepcopy(data)
        data["name"] = obj.theme_name  # keep inner name in sync with the editor identity
    ...
    def save(self, name: str | None = None, replace: bool = False, apply: bool = True) -> Path:
        ...
        synchronize_style(self.draft)
        # Save-as must rewrite the inner name too, or the legacy generator rejects
        # the file with "theme filename/name mismatch" and the unified contract,
        # wallpaper, and starship targets silently stop regenerating.
        self.draft["name"] = target_name
...
def list_themes(theme_dir: Path | None = None) -> list[str]:
    ...
    # Names starting with "_" are internal artifacts (e.g. the live-preview
    # draft); they must never surface as selectable themes.
