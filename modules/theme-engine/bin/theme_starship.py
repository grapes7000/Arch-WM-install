...
def render(colors, settings=None):
    ...
    # Section role overrides (hex or palette name) fall back to the theme's
    # own accent/accent2/urgent so the Prompt room's role fields always work.
    dir_color = str(p.get("directory_color") or colors["accent"])
    git_color = str(p.get("git_color") or colors["accent2"])
    status_color = str(p.get("status_color") or colors["urgent"])
...
format = "[{t('git_connector')}](fg:{dir_color} bg:{git_color})"

[custom.git_end]
command = "printf x"
when = "git rev-parse --is-inside-work-tree >/dev/null 2>&1"
format = "[{t('git_end')}](fg:{git_color})"'''
    ...
    # one_line puts the prompt character on the main line instead of a second
    # line; two_line keeps the Powerlevel10k-style split via $line_break.
    if bool(p.get("one_line", False)):
        main = (
            f"[{t('lead_fade')}](fg:surface){os_block}[ ](bg:surface)"
            f"[{t('lead_arrow')}](fg:surface bg:{dir_color})"
            f"$directory{git}${{custom.path_end}}"
            f"{dev}{container}{cloud}$memory_usage$character"
        )
    else:
        main = (
            f"[{t('lead_fade')}](fg:surface){os_block}[ ](bg:surface)"
            f"[{t('lead_arrow')}](fg:surface bg:{dir_color})"
            f"$directory{git}${{custom.path_end}}"
            f"{dev}{container}{cloud}$memory_usage"
...
success_symbol = ""
format = "[{t('cmd_status_icon')} $status ](bold fg:bg bg:{status_color})"
