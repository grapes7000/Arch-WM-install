...
def render_animations_lua(anim_name):
    ...
    hl.curve()/hl.animation() calls (see conf/defaults.lua), not the
    conf-file `animations { ... }` block, so the Lua theme module needs a
    translated form. Returns a list of Lua lines ready to append to a
    generated module.
    """
    if anim_name is None:
        return []
    ...
    for raw in anim["lines"]:
        ...
        if line.startswith("bezier ="):
            ...
            if len(parts) >= 5:
                name, x1, y1, x2, y2 = parts[:5]
                lines.append(
                    f'hl.curve("{name}", {{ type = "bezier", '
                    f'points = {{ {{ {x1}, {y1} }}, {{ {x2}, {y2} }} }} }})'
                )
        elif line.startswith("animation ="):
            parts = [part.strip() for part in line.split("=", 1)[1].split(",")]
            if len(parts) >= 4:
                leaf, onoff, speed, curve = parts[:4]
                style = parts[4] if len(parts) > 4 else ""
                enabled = "true" if onoff == "1" else "false"
                style_part = f', style = "{style}"' if style else ""
                lines.append(
                    f'hl.animation({{ leaf = "{leaf}", enabled = {enabled}, '
